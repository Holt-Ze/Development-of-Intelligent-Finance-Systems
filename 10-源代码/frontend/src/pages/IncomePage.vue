    ...
    <PageCard title="收入录入">
      <el-form :model="form" label-width="88px">
        ...
        <div class="page-actions">
          <el-button type="primary" :loading="submitting" @click="submitForm">{{ editingId ? '更新收入记录' : '保存收入记录' }}</el-button>
          <el-button @click="resetForm">重置</el-button>
        </div>
      </el-form>
    </PageCard>

    <PageCard title="收入记录列表">
...
const loadPageData = async () => {
  loading.value = true
  try {
    const [departments, accounts, categories, incomeResult] = await Promise.all([
      fetchList(endpointMap.departments),
      fetchList(endpointMap.accounts),
      fetchList(endpointMap.incomeCategories),
      fetchList(endpointMap.incomes)
    ])
    ...
    rows.value = incomeResult.records || []
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateItem(endpointMap.incomes, editingId.value, payload)
    } else {
      await createItem(endpointMap.incomes, payload)
    }
...
const changeStatus = async (row) => {
  try {
    await updateItemStatus(endpointMap.incomes, row.id, row.status === 1 ? 0 : 1)
