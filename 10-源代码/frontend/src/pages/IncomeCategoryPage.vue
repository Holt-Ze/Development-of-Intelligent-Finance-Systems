<template>
  <PageCard title="收入类别管理">
    <template #extra>
      <el-button type="primary" @click="openCreate">新增收入类别</el-button>
    </template>
  ...
  <el-dialog v-model="dialogVisible" :title="editingId ? '编辑收入类别' : '新增收入类别'" width="480px">
...
const loadRows = async () => {
  loading.value = true
  try {
    rows.value = await fetchList(endpointMap.incomeCategories)
  } catch (error) {
...
const submitForm = async () => {
  ...
  try {
    if (editingId.value) {
      await updateItem(endpointMap.incomeCategories, editingId.value, form)
    } else {
      await createItem(endpointMap.incomeCategories, form)
    }
...
const changeStatus = async (row) => {
  try {
    await updateItemStatus(endpointMap.incomeCategories, row.id, row.status === 1 ? 0 : 1)
