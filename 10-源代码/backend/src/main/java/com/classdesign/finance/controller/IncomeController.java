...
public class IncomeController {
    ...
    public IncomeController(RecordService recordService) {
    ...
    public ApiResponse<?> list(@RequestParam(required = false) Long deptId,
        ...
        return ApiResponse.success(recordService.listIncomes(deptId, accountId, categoryId, startDate, endDate));
    }

    @PostMapping
    public ApiResponse<?> create(@Valid @RequestBody FinanceRecordSaveRequest request) {
        return ApiResponse.success(recordService.createIncome(request));
    }

    @PutMapping("/{id}")
    public ApiResponse<?> update(@PathVariable Long id, @Valid @RequestBody FinanceRecordSaveRequest request) {
        return ApiResponse.success(recordService.updateIncome(id, request));
    }

    @PatchMapping("/{id}/status")
    public ApiResponse<?> updateStatus(@PathVariable Long id, @Valid @RequestBody StatusUpdateRequest request) {
        recordService.updateIncomeStatus(id, request.status());
        return ApiResponse.success(Map.of("message", "收入记录状态更新成功", "id", id, "status", request.status()));
    }
}
